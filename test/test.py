...
def readQuantity(browser, engine):
    try:
        value = browser.find_element(By.NAME, "inputdata").get_attribute("value")
        engine.say(value)
        engine.runAndWait()
    except:
...
def dockIN(browser, gate_pass, status, wb):
    #
...
def ASNReceipt(browser, wb, gate_pass, status, engine):
    #
    # ASN Receipts
    #
    try:
        while getCompleteStatus(wb) != 'Complete':
            ...
            submit(browser)
            enterGatePassID(browser, gate_pass, status)  # Enter the gate pass ID
            ...
            # if server gets stuck and times out refresh
            # try:
            # elem = WebDriverWait(browser, 120).until(EC.presence_of_element_located((By.ID, "Element_to_be_found")))  # This is a dummy element
            # except:
